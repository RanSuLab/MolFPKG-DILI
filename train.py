...
from models import MolFPKG_DILI
...
# Training settings
parser = argparse.ArgumentParser(description='MolFPKG_DILI')

parser.add_argument('--epochs', type=int, default=30, metavar='N',
                    help='number of epochs to train (default: 30)')
parser.add_argument('--model', type=str, default='MolFPKG_DILI',
                    help='Choose model : MolFPKG_DILI')
...
parser.add_argument('--opt', type=str, default='Adam',
                    help='optimizers')
parser.add_argument('--viz', type=str, default='MolFPKG_DILI',
                    help='visdom environment: MolFPKG_DILI')
...
def main(args, i, fp_data, data_path, save_res_path, save_model_path, ss):
    ...
    graph_data = torch.load(data_path + 'mol_graph.pt', weights_only=False)
    graph_dataloader = DataLoader(graph_data, batch_size=args.batchSize, shuffle=False)
    # kg_data = torch.load(data_path + 'graph.pt')
    kg_data = torch.load(data_path + 'graph_drugsim_08.pt', weights_only=False)
    ...
    if args.model == 'MolFPKG_DILI':
        model = MolFPKG_DILI(mlp_input_dim, mlp_hidden_dim, mlp_output_dim, gcn_input_dim, gcn_hidden_dim, gcn_output_dim, dim1, dim2, dim3, num_classes, args.dropout)
    ...
    '''
        ...
        verbose: 是否打印出信息
    '''
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min', factor=0.5, verbose=True, patience=5)
